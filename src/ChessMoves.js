export default function ChessMoves() {
    ...
    function checkForChecks( layout ) {
        return {
            whiteChecked: checkForCheck( 'w' ),
            redChecked: checkForCheck( 'r' )
        }

        function checkForCheck( side ) {
            var kingsSquare = findKing();
            return canEnemyAttackKing();

            function findKing() {
                for( let y = 0; y < layout.length; y++ ) {
                    for( let x = 0; x < layout[y].length; x++ ) {
                        if( layout[y][x].substring( 0, 2 ) === side + 'K' )
                            return [ x, y ];
                    }
                }
                console.warn( side + ' king not found' );
            }

            function canEnemyAttackKing() {
                const enemy = ( side === 'w' ) ? 'r' : 'w';

                for( let y = 0; y < layout.length; y++ ) {
                    for( let x = 0; x < layout[y].length; x++ ) {
                        if( layout[y][x][0] === enemy ) {
                            const pM = pieceMovements( x, y, layout );
                            for( let p = 0; p < pM.length; p++ ) {
                                if( pM[p][0] === kingsSquare[0] && pM[p][1] === kingsSquare[1] )
                                    return true;
                            }
                        }
                    }
